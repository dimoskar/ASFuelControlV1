...
namespace ASFuelControl.Box69
{
    public class Box69Protocol
    {
                       ...
                       //System.Diagnostics.Trace.WriteLine("Commad Read :"+ cmd.CommandType.ToString() + " --- " + cmd.Fpoint.AddressId.ToString() + " -- " + BitConverter.ToString(response));
                        switch (cmd.CommandType)
                        {
                            ...
                            case CommandTypeEnum.SendMainDisplayData:
                                Common.Enumerators.FuelPointStatusEnum newStatus = this.EvaluateStatus(response, cmd.Fpoint.Status);
                                ...
                                if (cmd.CommandType == CommandTypeEnum.RequestStatus && newStatus == Common.Enumerators.FuelPointStatusEnum.Nozzle && cmd.Fpoint.ActiveNozzle == null)
                                    ...
                                    cmd.Fpoint.Status = newStatus;
                                break;
                            case CommandTypeEnum.Halt:
                                Common.Enumerators.FuelPointStatusEnum newStatus1 = this.EvaluateStatus(response, cmd.Fpoint.Status);
                                cmd.Fpoint.Status = newStatus1;
                                cmd.Fpoint.QueryStop = false;
                                break;
                            case CommandTypeEnum.Authorize:
                                Common.Enumerators.FuelPointStatusEnum newStatus2 = this.EvaluateStatus(response, cmd.Fpoint.Status);
                                cmd.Fpoint.Status = newStatus2;
                            ...
                            case CommandTypeEnum.RequestActiveNozzle:
                                ...
                                cmd.Fpoint.Status = Common.Enumerators.FuelPointStatusEnum.Nozzle;
                            ...
                            case CommandTypeEnum.RequestID:
                                if (cmd.Fpoint.QueryTotals)
                                {
                                    ...
                                    if (this.TotalsRecieved != null)
                                        this.TotalsRecieved(cmd.Fpoint, new TotalsEventArgs(nz.ParentFuelPoint, nz.Index, nz.TotalVolume, 0));
                                }
                                break;
        ...
        private bool EvaluateTotals(Nozzle nozzle, byte[] buffer, bool ignoreError)
        {
            ...
            nozzle.ParentFuelPoint.Status = this.EvaluateStatus(status, nozzle.ParentFuelPoint.Status);
