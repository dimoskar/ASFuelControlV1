...
namespace PTSLib.PTS
{
    ...
    /// </summary>
    internal static class PtsCRC16
    {
        /// <summary>
        ...
        public static ushort Calculate(byte[] bytes)
        {
            ...
            for (int i = 0; i < bytes.Length; i++)
            {
                ...
                byte ml;
                ml = (byte)(code);          //place in ml a LSB of 16-teen digits number code (future СRС16)
                ml ^= bytes[i];             //make XOR between ml and msg[i] and place it in ml
                code &= 0xff00;             //null LSB of number code
                code += ml;
                for (int j = 0; j < 8; j++)
                {
                    ...
                    {                       //Least bit of code = 1
                        ...
                        code ^= polinom;    //operation XOR of variables code with polinom with placing of result into code
                    }
                    else
                    {                       //Least bit of code = 0
                        code >>= 1;         //shift right on 1 bit with assignment
        ...
        /// <param name="bytes">Message data bytes, from which CRC should becalculated.</param>
        /// <returns>Calculated CRC value.</returns>
        public static ushort CalculateFast(byte[] bytes)
        {
            ushort result = 0;

            for (int i = 0; i < bytes.Length; i++)
            {
                var index = (result & 0xFF) ^ bytes[i];
                result = (ushort)((result >> 8) ^ Crc16Table[index]);
            }
