    ...
    public class Connector
    {
        ...
        private void ThreadRun()
        {
            while (this.IsConnected)
            {
                try
                {
                    ...
                    {
                        this.serialPort.Write(new byte[] { (byte)probe.Address, 44 }, 0, 2);
        ...
        private void EvaluateResponse(string response)
        {
            if (response.Length < 27)
                return;
            if (System.IO.File.Exists("Unimep_" + CommunicationPort + ".txt"))
                System.IO.File.AppendText(response + "\r\n");

            int address = int.Parse(response.Substring(0, 2));
            int status = int.Parse(response.Substring(2, 1));
            decimal flevel = decimal.Parse(response.Substring(3, 7).Trim().Replace(".", ","));
            decimal wlevel = decimal.Parse(response.Substring(11, 7).Trim().Replace(".", ","));
            decimal temp = decimal.Parse(response.Substring(20, 5).Trim().Replace(".", ","));
