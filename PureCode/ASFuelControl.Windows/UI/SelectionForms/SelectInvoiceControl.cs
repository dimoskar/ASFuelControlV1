...
namespace ASFuelControl.Windows.UI.SelectionForms
{
    public partial class SelectInvoiceControl : UserControl
    {
        ...
        public void LoadData()
        {
            ...
            var q1 = this.database.InvoiceLines.Where(il => !il.TankFillingId.HasValue && il.Invoice.InvoiceType.TransactionType == 1 && il.FuelTypeId == this.FuelTypeId && 
                il.Invoice.TransactionDate.Date <= dateTo && il.Invoice.TransactionDate.Date >= dateFrom);
            ...
            var q3 = this.database.InvoiceLines.Where(il => !il.TankFillingId.HasValue && il.Invoice.InvoiceType.TransactionType == 0 && il.Invoice.InvoiceType.IsInternal.HasValue &&
                il.Invoice.InvoiceType.IsInternal.Value && il.Invoice.InvoiceTypeId != literCheckGuid && il.FuelTypeId == this.FuelTypeId && il.TankId == this.TankId &&
                il.Invoice.TransactionDate.Date <= dateTo && il.Invoice.TransactionDate.Date >= dateFrom);
        ...
        private void invoiceLineRadGridView_CommandCellClick(object sender, EventArgs e)
        {
            ...
            else if (cell.ColumnInfo.Name == "openInvoice")
            {
                ...
                Forms.InvoiceForm iform = new Forms.InvoiceForm();
