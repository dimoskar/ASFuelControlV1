...
namespace ASFuelControl.Windows.Threads
{
    ...
    public class ThreadController
    {
        ...
        private int CurrentDBVersion = 22;
        ...
        private void InitializeControllers()
        {
            ...
            foreach (Data.Dispenser dispenser in this.database.Dispensers)
            {
                foreach (Data.Nozzle nozzle in dispenser.Nozzles)
                {
                    ...
                    int invalidSales = nozzle.SalesTransactions.Where(s => s.InvalidSale.HasValue && s.InvalidSale.Value).Count();
                    if (invalidSales > 0)
                    {
                        vDisp.HasInvalidSale = true;
                        vDisp.InvalidSales.AddRange(nozzle.SalesTransactions.Where(s => s.InvalidSale.HasValue && s.InvalidSale.Value).Select(s => s.SalesTransactionId).ToArray());
                    }
                }
            }
