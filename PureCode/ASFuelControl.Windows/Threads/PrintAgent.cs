    ...
    public class PrintAgent
    {
        ...
        public static void PrintInvoiceDirect(Data.Invoice invoice)
        {
            if (invoice.InvoiceType.IsLaserPrint.HasValue && invoice.InvoiceType.IsLaserPrint.Value)
            {
                ...
                {
                    Reports.Invoices.IncoiceReportShort report = new Reports.Invoices.IncoiceReportShort();
                    report.DataSource = db.InvoiceLines.Where(il => il.InvoiceId == invoice.InvoiceId).ToList();
        ...
        private string[] CreateInvoiceText(Data.Invoice invoice)
        {
            ...
            foreach(Data.InvoiceLine invLine in invoice.InvoiceLines)
            {
                ...
                string details = invLine.FuelType.Name.LeftString(18) + invLine.Volume.ToString("N2").CenterString(7) + invLine.UnitPrice.ToString("N3").CenterString(6) + (invLine.TotalPrice - invLine.VatAmount).ToString("N2").CenterString(7) + invLine.VatAmount.ToString("N2").CenterString(6);
                invoiceLines.Add(details.CenterString(printerLineWidth));
        ...
        private void ThreadRun()
        {
            ...
            while(agentRunning)
            {
                ...
                try
                {
                    ...
                    var qFillings = this.database.TankFillings.Where(t => (t.SignSignature == null || t.SignSignature == "") && t.InvoiceLines.Count > 0);
        ...
        /// <summary>
        /// Applies Sign to the given nentry in the database
        /// </summary>
        ...
        public void PrintToPrinter(Data.Balance balance, bool sendMail)
        {
            ...
            if(balance.StartDate.Date != balance.EndDate.Date)
        ...
        private void PrintToPrinterInternalInvoice(Data.Invoice invoice, bool showAdditionalData)
        {
            Data.DatabaseModel db = new Data.DatabaseModel(Properties.Settings.Default.DBConnection);
            if (invoice.InvoiceType.IsInternal.HasValue && invoice.InvoiceType.IsInternal.Value)
            {
