...
namespace ASFuelControl.MillenniumMulti
{
    ...
    public class MillenniumMultiConnector: Common.IFuelProtocol
    {
        ...
        public void Connect()
        {
            ...
            {
                if (System.IO.File.Exists("MillenuiumMulti.log"))
                    System.IO.File.AppendAllText("MillenuiumMulti.log", ex.Message + "\r\n");
            }
        }
                ...
                try
                {
                    foreach(Common.FuelPoint fp in this.fuelPoints)
                    {
                        ...
                        try
                        {
                            ...
                            if (System.IO.File.Exists("MillenuiumMulti.log"))
                                System.IO.File.AppendAllText("MillenuiumMulti.log", string.Format("Address : {0}, Channel : {1}", fp.Address, fp.Channel) + "\r\n");
        ...
        public void GetTotals(Common.Nozzle nz)
        {
            ...
            this.executeCommand(this.cmd, nz.ParentFuelPoint);
        }
        ...
        public void AuthorizeVolume(Common.FuelPoint fp)
        {
            ...
            //CreateCmd(fp.Address, fp.Channel, CommandType.Preset, amount);
            //this.executeCommand(this.cmd, fp, false);
        }
        public void AuthorizeAmount(Common.FuelPoint fp)
        {
            ////Mexri 2 dekadika
            //CreateCmd(fp.Address, fp.Channel, CommandType.Preset, (int)fp.PresetAmount * 100,;
            //this.executeCommand(this.cmd, fp, false);
        }
        ...
        public void GetLastSalesId(Common.Nozzle nz) //GETTOTALS
        {  
        ...
        {
            int id = (int)nz.GetExtendedProperty("LastSalesId", 0);
            if(id != 0)
            {
                try
                {
        ...
        private void evaluateBuffer(List<byte> buffer)
        {
            ...
            try
            {
                ...
                {
                    if (System.IO.File.Exists("MilleniumMulti.log"))
                        System.IO.File.AppendAllText("MilleniumMulti.log", "RX : " + BitConverter.ToString(response) + "\r\n");
                    ...
                    switch(type)
                    {
                        case Response.SelfStatus:
                            ...
                            tnz = tfp.Nozzles.Where(x => (x.NozzleIndex == nzIndexA)).FirstOrDefault();
        ...
        private void executeCommand(byte[] cmd, Common.FuelPoint fp)
        {
            ...
            if (System.IO.File.Exists("MilleniumMulti.log"))
                System.IO.File.AppendAllText("MilleniumMulti.log", "TX : " + BitConverter.ToString(cmd) + "\r\n");
            ...
            if (System.IO.File.Exists("MillenuiumMulti.log"))
                System.IO.File.AppendAllText("MillenuiumMulti.log", "RX : " + BitConverter.ToString(this.buffer.ToArray()) + "\r\n");
            evaluateBuffer(this.buffer);
            //foo.ComBuffer.Add(this.buffer.ToArray());
        }
        ...
        private void CreateCmd(int dispenserId, int fuelPointChannel,int nzIndex, CommandType command, int price)
        {
            ...
            if(command == CommandType.SendPrices)
            {
                ...
                if (nzIndex==1)
                    buffer = new byte[] { 0xFD, (byte)(dispenserId), 0x00, 0x02, 0x01, 0x00, 0x40, 0x00, 0x0D, 0x06, 0x61, 0x00, 0x00, 0x00, 0x01, 0x11, 0x02, 0x04, 0x03, 0x00, (byte)ith(p1), (byte)ith(p2), 0xFC };
                else if (nzIndex>1)
                      buffer = new byte[] { 0xFD, (byte)(dispenserId), 0x00, 0x02, 0x01, 0x00, 0x40, 0x00, 0x0D, 0x06, 0x61, 0x00, 0x00, 0x00, 0x05, 0x11, 0x02, 0x04, 0x03, 0x00, (byte)ith(p1), (byte)ith(p2), 0xFC };
                
                   // buffer = new byte[] { 0xFD, (byte)(dispenserId), 0x00, 0x02, 0x01, 0x00, 0x40, 0x00, 0x0D, 0x06, (byte)(96 + fuelPointChannel), 0x00, 0x00, 0x00, (byte)1, (byte)(16 + nzIndex), 0x02, 0x04, 0x03, 0x00, (byte)ith(p1), (byte)ith(p2), 0xFC };
        ...
        {   //input buffer    FD xx ... xx FC
            ...
            else
            {
                ...
                if (System.IO.File.Exists("MillenuiumMulti.log"))
                    System.IO.File.AppendAllText("MillenuiumMulti.log", "TX : " + BitConverter.ToString(this.cmd) + "\r\n");
            }
