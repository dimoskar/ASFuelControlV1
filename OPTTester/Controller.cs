...
namespace ASFuelControl.Unixfor
{
    public class Controller
    {
        ...
        public FuelPump[] FuelPumps { set; get; }
        ...
        {
            string resp = string.Format("{0}|{1}|{2}|{3}|{4}", parms[1], parms[2], parms[3], "0", unixforVersion);
            int len = CaclulateLength(resp);
            return len.ToString() + "|" + resp;
        }

        public string KeepAlive(string[] parms)
        {
            string resp = string.Format("{0}|{1}|{2}", parms[1], parms[2], parms[3]);
            int len = CaclulateLength(resp);
            return len.ToString() + "|" + resp;
        }
        ...
        public string PumpStatus(string[] parms)
        {
            ...
            var pump = this.FuelPumps.FirstOrDefault(f => f.PumpIndex == pumpIndex);
            if(pump == null)
            {
                return string.Format("{0}|{1}|{2}|{3}", parms[0], parms[1], parms[2], parms[3]);
            }
        ...
        private int CaclulateLength(string data)
        {
            int len = data.Length;
            int len2 = (len.ToString() + "|" + data).Length;
            if (len + (len.ToString() + "|").Length == len2)
                return len2;
            else
                return len2 + 1;
        }
