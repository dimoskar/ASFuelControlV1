...
namespace ASFuelControl.WayneDL
{
    public class WayneDLProtocol : Common.IFuelProtocol
    {
        ...
        private byte[] ExecuteCommand(int responseLength, byte[] cmd)
        {
            ...
            if (System.IO.File.Exists("WayneDL.txt"))
            {
                string logString = string.Format("{1:dd/MM/yyyy HH:mm:ss.fff} WR:{0}\r\n", str1, DateTime.Now);
                System.IO.File.AppendAllText("WayneDL.txt", logString);
            }
            ...
            if (upResponse.Length > 0 && System.IO.File.Exists("WayneDL.txt"))
            {
                string strupResp = BitConverter.ToString(upResponse);
                string logString = string.Format("{1:dd/MM/yyyy HH:mm:ss.fff} RD:{0}\r\n", strupResp, DateTime.Now);
                System.IO.File.AppendAllText("WayneDL.txt", logString);
            }
