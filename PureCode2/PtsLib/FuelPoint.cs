...
namespace PTSLib.PTS
{
    ...
    public class FuelPoint
    {
        ...
        {
            if (e.CurrentStatus == FuelPointStatus.NOZZLE)
            {
                this.DispensedVolume = 0;
                this.DispensedAmount = 0;
            }
        ...
        protected internal void OnTotalsUpdated(TotalsEventArgs e)
        {
            ...
            if (!this.Initialized)
            {
                if (!e.Nozzle.NozzleInitialized && e.Nozzle.TotalDispensedVolume > 0)
                {
                    ...
                    if(e.Nozzle.FuelPoint.Nozzles.Where(n=>n.NozzleInitialized).Count() == e.Nozzle.FuelPoint.Nozzles.Where(n=>n.NozzleAlive).Count())
