...
namespace ASFuelControl.PTS
{
    public class PtsConnector : Common.IController
    {
        ...
        public void AddFuelPoint(int channel, int address, int nozzleCount, int amountDecimalPlaces, int volumeDecimalPlaces, int untiPriceDecimalPlaces, int totalDecimalPlaces)
        {
            ...
            fp.AmountDecimalPlaces = amountDecimalPlaces;
            fp.UnitPriceDecimalPlaces = untiPriceDecimalPlaces;
            fp.TotalDecimalPlaces = totalDecimalPlaces;
            if (this.internalQueue.ContainsKey(fp))
                return;
            this.internalQueue.TryAdd(fp, new ConcurrentQueue<ASFuelControl.Common.FuelPointValues>());
            this.fuelPoints.Add(fp);
        ...
        public void AddFuelPoint(int channel, int address, int nozzleCount, int decimalPlaces, int untiPriceDecimalPlaces)
        {
            ...
            fp.Channel = channel;
            fp.NozzleCount = nozzleCount;
            fp.VolumeDecimalPlaces = decimalPlaces;
            fp.AmountDecimalPlaces = decimalPlaces;
            fp.UnitPriceDecimalPlaces = untiPriceDecimalPlaces;
            if (this.internalQueue.ContainsKey(fp))
                return;
            this.internalQueue.TryAdd(fp, new ConcurrentQueue<ASFuelControl.Common.FuelPointValues>());
        ...
        void fp_TotalsUpdated(object sender, PTSLib.PTS.TotalsEventArgs e)
        {
            ...
            Common.Sales.SaleData sale = nz.saleHandler.AddTotalizer((decimal)e.Nozzle.TotalDispensedVolume, 
                ...
                (decimal)e.Nozzle.FuelPoint.DispensedVolume / (decimal)Math.Pow(10, fp.VolumeDecimalPlaces), 
                (decimal)e.Nozzle.FuelPoint.DispensedAmount / (decimal)Math.Pow(10, fp.AmountDecimalPlaces));
        ...
        void fp_DispensedValuesChanged(object sender, EventArgs e)
        {
            ...
            Common.FuelPointValues values = new Common.FuelPointValues();
            values.CurrentVolume = fp.DispensedVolume / (decimal)Math.Pow(10, fuelPoint.VolumeDecimalPlaces);
            values.CurrentPriceTotal = (decimal)fp.DispensedAmount  * (100 / (decimal)Math.Pow(10, fuelPoint.UnitPriceDecimalPlaces)) / (decimal)Math.Pow(10, fuelPoint.AmountDecimalPlaces);
