...
namespace ASFuelControl.Box69
{
    public class Box69Protocol
    {
            ...
            while (true)
            {
                ...
                try
                {
                    foreach (FuelPoint fp in this.fuelPoints)
                    {
                        ...
                        if (fp.QuerySetPrice && fp.Nozzles.Where(n=>n.UnitPrice > 0).Count() > 0)
                        {
        ...
        {
            Common.Enumerators.FuelPointStatusEnum oldstatus = fp.DispenserStatus;
            ...
            Common.Enumerators.FuelPointStatusEnum newStatus =  this.EvaluateStatus(response, fp.Status);
             if(oldstatus != newStatus)
            {
                if(this.StatusChanged != null)
                    this.StatusChanged(this, new StatusChangedEventArgs(fp , newStatus, oldstatus));
            }
             return newStatus;
        }
        ...
        private Common.Enumerators.FuelPointStatusEnum InitializeFuelPoint(FuelPoint fp)
        {
            ...
            byte[] response = this.ExecuteCommnad(cmd);
            //Common.Enumerators.FuelPointStatusEnum oldstatus = fp.;
            if (response.Length == 0)
            return fp.Status;
             this.EvaluateStatus(response, fp.Status);
            return fp.Status;
        }
        ...
        private bool EvaluateTotals(Nozzle nozzle, byte[] buffer, bool ignoreError)
        {
            ...
            if(nozzle.Totalizer > 0) nozzle.ParentFuelPoint.Initialized = true;

            if(this.TotalsRecieved != null && nozzle.ParentFuelPoint.Initialized == true)
                nozzle.Status = nozzle.ParentFuelPoint.Status;
