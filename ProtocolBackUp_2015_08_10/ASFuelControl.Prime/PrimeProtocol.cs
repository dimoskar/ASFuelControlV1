...
namespace ASFuelControl.Prime
{
    public class PrimeProtocol : Common.IFuelProtocol
    {
                    ...
                    foreach (Common.FuelPoint fp in this.fuelPoints)
                    {
                        ...
                        try
                        {
                            ...
                            if (fp.Status == Common.Enumerators.FuelPointStatusEnum.TransactionCompleted)
                                GetSale(fp.Nozzles[0]);

                            else if ((oldstatus == Common.Enumerators.FuelPointStatusEnum.Work || oldstatus == Common.Enumerators.FuelPointStatusEnum.Nozzle) && fp.Status == Common.Enumerators.FuelPointStatusEnum.Idle)
                                GetSale(fp.Nozzles[0]);

                            if (oldstatus == Common.Enumerators.FuelPointStatusEnum.Work && (fp.Status == Common.Enumerators.FuelPointStatusEnum.Nozzle || fp.Status == Common.Enumerators.FuelPointStatusEnum.Idle))
                            {
                                GetSale(fp.Nozzles[0]);
        ...
        private bool ExecuteCommand(int responseLength, byte[] command, bool awaitAck)
        {
            ...
            while (this.serialPort.ReadBufferSize < respLen || waiting < 300)
                waiting = waiting + 10;
        ...
        {
            this.serialPort.Write(new byte[] { 05, 30, 31 }, 0, 3);
        }


        private void GetTotals(Nozzle nz)
        {
            byte[] command = this.BuildCommand(nz.ParentFuelPoint, "CQ", new byte[] { });
        ...
        private void Authorize(FuelPoint fp)
        {
            if (fp.PresetAmount < 999999)
            {
                ...
                int i = 0;
                while (!this.ExecuteCommand(0, cmd, true) && i < 300)
                {
        ...
        private void GetDisplayData(FuelPoint fp, byte[] dataBuffer)
        {
            ...
            fp.DispensedAmount = decimal.Parse(priceData) / Convert.ToDecimal(Math.Pow(10, fp.AmountDecimalPlaces));
            fp.DispensedVolume = decimal.Parse(volumeData) / Convert.ToDecimal(Math.Pow(10, fp.VolumeDecimalPlaces));
            if (this.DataChanged != null)
            {
                Common.FuelPointValues values = new Common.FuelPointValues();
                values.CurrentSalePrice = fp.ActiveNozzle.UnitPrice;
                values.CurrentPriceTotal = fp.ActiveNozzle.ParentFuelPoint.DispensedAmount;
                values.CurrentVolume = fp.ActiveNozzle.ParentFuelPoint.DispensedVolume;
        ...
        private bool GetTrancactionEndsData(FuelPoint fp, byte[] dataBuffer, bool fireEvent)
        {
            try
            {
                ...
                decimal unitPrice = decimal.Parse(unitPriceData) / Convert.ToDecimal(Math.Pow(10, fp.UnitPriceDecimalPlaces));
                Thread.Sleep(70);
                ...
                if (fireEvent)
                {
                    ...
                    {
                        this.TotalsRecieved(this, new Common.TotalsEventArgs(fp, fp.Nozzles[0].Index, fp.Nozzles[0].TotalVolume, fp.Nozzles[0].TotalPrice));
        ...
        private bool GetTotalsData(FuelPoint fp, byte[] dataBuffer)
        {
            try
            {
                ...
                fp.Nozzles[0].TotalVolume = decimal.Parse(totalVolume) * 1000 / Convert.ToDecimal(Math.Pow(10, fp.UnitPriceDecimalPlaces));
                if (fp.DispensedVolume == 0 && fp.Nozzles[0].LastTotalVolume < fp.Nozzles[0].TotalVolume)
                    fp.DispensedVolume = fp.Nozzles[0].TotalVolume - fp.Nozzles[0].LastTotalVolume;
                fp.Nozzles[0].TotalPrice = fp.Nozzles[0].TotalPrice + ((fp.DispensedVolume * fp.Nozzles[0].UnitPrice) * Convert.ToDecimal(Math.Pow(10, fp.UnitPriceDecimalPlaces)));
            ...
            while (true)
            {
                ...
                switch (command)
                {
                    ...
                    case "TR":
                        ...
                        if (oldStatus == Common.Enumerators.FuelPointStatusEnum.Offline)
                            displayOk = this.GetTrancactionEndsData(fp, dataBuffer.Skip(textStart + 3).Take(18).ToArray(), true);
                        else
                            displayOk = this.GetTrancactionEndsData(fp, dataBuffer.Skip(textStart + 3).Take(18).ToArray(), false);
                        if (displayOk)
                            this.serialPort.Write(new byte[] { ACK }, 0, 1);
                        else
                ...
                if (!fp.QueryAuthorize)
                {
                    ...
                    if (oldStatus == fp.Status && fp.Status == Common.Enumerators.FuelPointStatusEnum.Nozzle)
                        fp.Status = Common.Enumerators.FuelPointStatusEnum.Idle;
                }
                if (oldStatus != fp.Status)
                {
                    ...
                    System.IO.File.AppendAllText(System.Environment.CurrentDirectory + "\\logs" + "\\PrimeTotals.txt", "\r\n" + fp.Status.ToString());


                    if (this.DispenserStatusChanged != null)
                        this.DispenserStatusChanged(this, new FuelPointValuesArgs() { CurrentFuelPoint = fp, CurrentNozzleId = 0, Values = new FuelPointValues() { ActiveNozzle = -1, Status = fp.Status } });
                }
        ...
        private void GetSale(Nozzle nz)
        {
            byte[] command = this.BuildCommand(nz.ParentFuelPoint, "TQ", new byte[] { });
            this.ExecuteCommand(30, command, false);
