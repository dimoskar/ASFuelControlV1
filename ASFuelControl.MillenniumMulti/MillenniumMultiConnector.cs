...
namespace ASFuelControl.MillenniumMulti
{
    ...
    public class MillenniumMultiConnector : Common.IFuelProtocol
    {
                    ...
                    foreach (Common.FuelPoint fp in this.fuelPoints)
                    {
                        ...
                        try
                        {
                            ...
                            if (fp.QueryAuthorize)
                            {
                                ...
                                this.PostWork(fp);
                                System.Threading.Thread.Sleep(15);
                                //if(fp.PresetVolume == 0 && fp.PresetAmount == 0)
                                //{
                                //    fp.PresetVolume = max;
                                //    fp.PresetAmount = max;
                                //}
                                ...
                                else if(fp.PresetAmount == 0 && fp.PresetVolume == 0 && !fp.OTPEnabled)
                                    this.Authorize(fp.ActiveNozzle);
                                else if (fp.PresetAmount != max && fp.PresetVolume == 0)
                                    this.AuthorizeAmount(fp.ActiveNozzle);
                                else if (fp.PresetVolume != max && fp.PresetAmount == 0)
                                    this.AuthorizeVolume(fp.ActiveNozzle);
                                else if (fp.PresetVolume < 0 && fp.PresetAmount < 0)
                                    this.Authorize(fp.ActiveNozzle);
        ...
        {
            CreateCmd(nz.ParentFuelPoint.Address, nz.ParentFuelPoint.Channel, nz.Index, CommandType.RequestTotals);
        ...
        public void AuthorizeVolume(Common.Nozzle nz)
        {
            ...
            CreateCmd(nz.ParentFuelPoint.Address, nz.ParentFuelPoint.Channel, nz.NozzleSocket, CommandType.Preset, (int)nz.ParentFuelPoint.PresetAmount * 100);
        ...
        {
            CreateCmd(nz.ParentFuelPoint.Address, nz.ParentFuelPoint.Channel, nz.NozzleSocket, CommandType.Authorize);
        ...
        {
            CreateCmd(nz.ParentFuelPoint.Address, nz.ParentFuelPoint.Channel, nz.NozzleSocket, CommandType.GetLastSalesId);
            this.executeCommand(this.cmd, nz.ParentFuelPoint, 40);
        }
        public void LastTransactionInfo(Common.Nozzle nz)
        {
            ...
            if (id != 0)
            {
                try
                {
                    CreateCmd(nz.ParentFuelPoint.Address, nz.ParentFuelPoint.Channel, nz.Index, CommandType.InfoForId, id);
                    this.executeCommand(this.cmd, nz.ParentFuelPoint, 40);
                ...
                foreach (byte[] response in buffer.ToArray().ValidResponses())
                {
                    ...
                    switch (type)
                    {
                        ...
                        case Response.SelfStatus:
                            ...
                            int nzMatchIndex = nzIndexA > 0 ? (int)Math.Log(nzIndexA, 2) + 1 : 0;
                            tfp.Status = evalStatus((int)response[15]);
                            tfp.DispenserStatus = tfp.Status;
                            Nozzle tnzSock = tfp.Nozzles.Where(x => (x.NozzleSocket == nzMatchIndex)).FirstOrDefault();
                            ...
                            if ((tfp.Status != oldStatus || (tnzSock != null && tfp.ActiveNozzleIndex != tnzSock.Index - 1)) && this.DispenserStatusChanged != null)
                            {
                                Common.FuelPointValues values = new Common.FuelPointValues();
                                if (tfp.Status != Common.Enumerators.FuelPointStatusEnum.Close && tfp.Status != Common.Enumerators.FuelPointStatusEnum.Idle && tfp.Status != Common.Enumerators.FuelPointStatusEnum.Offline)
                                {
                                    ...
                                    tfp.ActiveNozzleIndex = tnzSock.Index - 1;
                                    values.ActiveNozzle = tnzSock.Index - 1;
                                }
                                else
                                {
                                ...
                                this.DispenserStatusChanged(this, new Common.FuelPointValuesArgs()
                                {
                                    ...
                                    CurrentNozzleId = nzMatchIndex != 0 ? nzMatchIndex - 1 : 0,
                                    Values = values
                                });
                        ...
                        case Response.Display:
                            ...
                            if (flag >= 6)
                            {
                                tfp.ActiveNozzle.SetExtendedProperty("LastSalesId", (int)response.skip(33).takeToDecimal(2));
                            }
                            CallEventDataChanged(tfp);
                            break;
                        #endregion
                        ...
                        case Response.LastSalesId:
                            ...
                            int nzMatchIndexL = nzIndexL > 0 ? (int)Math.Log(nzIndexL, 2) + 1 : 0;

                            //nzIndexL = NozzleIndexForHumans.FirstOrDefault(x => x.Value == nzIndexL).Key;
                            Nozzle tn = tfp.Nozzles.Where(x => (x.NozzleSocket == nzIndexL)).FirstOrDefault();
                            lock (tn)
                            {
                                tn.TotalVolume = response.skip(15).takeToDecimal(6);
                                tn.TotalPrice = response.skip(24).takeToDecimal(6);
                                tn.SetExtendedProperty("LastSalesId", (int)response.skip(37).takeToDecimal(2));
                                tn.SetExtendedProperty("GotTotals", true);
                            }
                        ...
                        case Response.Totals:
                            ...
                            int nzMatchIndex1 = nzIndexT > 0 ? (int)Math.Log(nzIndexT, 2) + 1 : 0;
                            tnz = tfp.Nozzles.Where(x => (x.Index == nzMatchIndex1)).FirstOrDefault();
                            ...
                            //{
                            if (this.TotalsRecieved != null)
                                this.TotalsRecieved(this, new Common.TotalsEventArgs(tfp, tnz.Index, tnz.TotalVolume, tnz.TotalPrice));
                            //}
                        ...
                        case Response.Status2:
                            ...
                            int nzMatchIndex2 = nzIndexS > 0 ? (int)Math.Log(nzIndexS, 2) + 1 : 0;
                            if (nzIndexS != 0)
                                tnz = tfp.Nozzles.Where(x => (x.Index == nzIndexS)).FirstOrDefault();
                            ...
                            if ((tfp.Status != oldStatus || (nzIndexS > 0 && (tnz != null && tfp.ActiveNozzleIndex != tnz.Index - 1))) && this.DispenserStatusChanged != null)
                            {
                                Common.FuelPointValues values = new Common.FuelPointValues();
                                if (tfp.Status != Common.Enumerators.FuelPointStatusEnum.Close && tfp.Status != Common.Enumerators.FuelPointStatusEnum.Idle && tfp.Status != Common.Enumerators.FuelPointStatusEnum.Offline)
                                {
