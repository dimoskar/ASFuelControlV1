...
namespace ASFuelControl.Box69
{
    public class Controller : Common.IController
    {
        ...
        void fp_TotalsRecieved(object sender, TotalsEventArgs e)
        {
            ...
            {
                if (e.Nozzle.Totalizer == 0 && nz.CurrentSale.TotalizerStart > 0)
                {
                    nz.QueryTotals = true;
                    return;
                }
                ...
                decimal price = (decimal)fuelPoint.Nozzles[nz.Index - 1].UnitPrice / (decimal)Math.Pow(10, fuelPoint.PriceDecimalPlaces);
                if (e.Nozzle.Totalizer <= nz.CurrentSale.TotalizerStart)
                {
                    nz.QueryTotals = true;
                    fp.QueryTotals = true;
                ...
                nz.CurrentSale.TotalizerEnd = e.Nozzle.Totalizer;
                nz.CurrentSale.TotalVolume = (nz.CurrentSale.TotalizerEnd - nz.CurrentSale.TotalizerStart) /(decimal) Math.Pow(10, fp.DecimalPlaces);

                if (nz.CurrentSale.TotalizerStart < 0)
                    nz.CurrentSale.TotalVolume = 0;
                
                nz.QueryTotals = false;
                nz.CurrentSale.TotalPrice = decimal.Round((decimal)nz.CurrentSale.TotalVolume * price, fp.DecimalPlaces);
