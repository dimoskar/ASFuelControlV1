    ...
    public class FuelPumpWorkFlow : WorkFlow.IWorkFlow, IFuelPumpWorkFlow
    {
        ...
        public void SetValues(object _values)
        {
            ...
            if (values.ActiveNozzle >= 0 && values.Status != Common.Enumerators.FuelPointStatusEnum.Idle && values.Status != Common.Enumerators.FuelPointStatusEnum.Offline)
            {
                ...
                if (values.CurrentSalePrice != this.Dispenser.ActiveNozzle.CurrentSaleUnitPrice && values.CurrentSalePrice > 0)
                    this.Dispenser.ActiveNozzle.CurrentSaleUnitPrice = values.CurrentSalePrice;
                if (values.CurrentPriceTotal != this.Dispenser.ActiveNozzle.CurrentSaleTotalPrice && values.CurrentPriceTotal > 0)
                    this.Dispenser.ActiveNozzle.CurrentSaleTotalPrice = values.CurrentPriceTotal;
                if (values.CurrentVolume != this.Dispenser.ActiveNozzle.CurrentSaleTotalVolume && values.CurrentVolume > 0)
                {
                    this.Dispenser.ActiveNozzle.CurrentSaleTotalVolume = values.CurrentVolume;
                    //this.Dispenser.ActiveNozzle.TotalVolumeCounter = this.Dispenser.ActiveNozzle.LastVolumeCounter + decimal.Round(100 * values.CurrentVolume, 0);
                }
