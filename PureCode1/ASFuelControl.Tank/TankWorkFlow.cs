    ...
    public class TankWorkFlow : WorkFlow.IWorkFlow, ITankWorkFlow
    {
        ...
        public void ValidateTransitions()
        {
            if (!this.Tank.HasChanges && !this.IsOnSale(null))
                return;
            if(this.HasAlerts() && !this.QueryExtraction(null) && !this.QueryFilling(null))
                return;
        ...
        private bool IsOnSale(object foo)
        {
            ...
            {
                if ((nozzle.ParentDispenser.Status == Common.Enumerators.FuelPointStatusEnum.Work || nozzle.ParentDispenser.Status == Common.Enumerators.FuelPointStatusEnum.TransactionCompleted) && nozzle.ParentDispenser.ActiveNozzle == nozzle)
                {
                    return true;
                }
            }
        ...
        private bool IsIdle(object _status)
        {
            if (this.Process.CurrentState == this.sallingState)
            {
                foreach (VirtualDevices.VirtualNozzle nozzle in this.Tank.ConnectedNozzles)
                {
                    if (nozzle.ParentDispenser.Status == Common.Enumerators.FuelPointStatusEnum.Work || nozzle.ParentDispenser.Status == Common.Enumerators.FuelPointStatusEnum.TransactionCompleted)
                        return false;
                }
            }
            bool isIdle = false;
