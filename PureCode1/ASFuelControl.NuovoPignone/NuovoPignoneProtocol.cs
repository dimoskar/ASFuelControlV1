...
namespace ASFuelControl.NuovoPignone
{
    public class NuovoPignoneProtocol : Common.IFuelProtocol
    {
                        ...
                        try
                        {
                            ...
                            if (fp.Status == Common.Enumerators.FuelPointStatusEnum.Work)
                            {
                                ...
                                if(this.DataChanged != null)
                                {
                                    ...
                                    this.DataChanged(this, new Common.FuelPointValuesArgs() { CurrentFuelPoint =fp, CurrentNozzleId = fp.ActiveNozzleIndex + 1, Values=values});
                                }
                            }
        ...
        private Common.Nozzle evalDisplay(Common.Nozzle nozzle, byte[] response)
        {
            ...
            for (int i = 1; i < 8; i++)
            {
                if (StringToByteArray(parms[2 * i])[0] + StringToByteArray(parms[2 * i + 1])[0] != 255) { throw new Exception("evalDisplay() Failed"); }
            }
        ...
        {   //nz1
            ...
            for (int i = 1; i < 7; i++)
            {
                if (StringToByteArray(parms[2 * i])[0] + StringToByteArray(parms[2 * i + 1])[0] != 255) { throw new Exception("evalTotals() Failed"); }
            }
        ...
        private Common.FuelPoint evaluateStatus(Common.FuelPoint fp, byte[] response)
        {
            ...
            {
                byte b1 = response[2];
                if (b1 == 0xFF) { fp.Status = Common.Enumerators.FuelPointStatusEnum.Idle; }
                else if (b1 == 0xF7) { fp.Status = Common.Enumerators.FuelPointStatusEnum.Nozzle; }
                else if (b1 == 0xF5) { fp.Status = Common.Enumerators.FuelPointStatusEnum.Work; }
                else if (b1 == 0xBF) { fp.Status = Common.Enumerators.FuelPointStatusEnum.TransactionCompleted; }
                return fp;
            }
        }
        #endregion
        ...
        public Common.Nozzle GetUnitPrice(Common.Nozzle nozzle)
        {
            ...
            while (this.serialPort.BytesToRead < (int)responseLength.unitprice)
            {
                System.Threading.Thread.Sleep(15); //15*4 = 70 [ NP 68ms response time]
            }
            byte[] upResponse = new byte[this.serialPort.BytesToRead];
        ...
        public bool AuthorizeFuelPoint(Common.FuelPoint fp)
        {
            try
            {
                ...
                this.serialPort.Write(buffer, 0, buffer.Length);
                while (this.serialPort.BytesToRead < (int)responseLength.auth)
                {
                    System.Threading.Thread.Sleep(20);
                }
                byte[] response = new byte[this.serialPort.BytesToRead];
