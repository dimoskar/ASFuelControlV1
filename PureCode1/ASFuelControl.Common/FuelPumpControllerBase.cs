...
namespace ASFuelControl.Common
{
    public class FuelPumpControllerBase : IController
    {
        ...
        public virtual void AddFuelPoint(int channel, int address, int nozzleCount, int decimalPlaces, int untiPriceDecimalPlaces)
        {
        ...
        protected virtual void OnTransactionCompleted(TotalsEventArgs e)
        {
            ...
            if (!nz.CurrentSale.SaleCompleted)
            {
                ...
                if (e.CurrentFuelPoint.DispensedVolume != totalVolume)
                {
                ...
                {
                    nz.CurrentSale.TotalPrice = e.CurrentFuelPoint.DispensedAmount;
                    nz.CurrentSale.TotalVolume = e.CurrentFuelPoint.DispensedVolume;
                }
                //System.IO.File.AppendAllText(System.Environment.CurrentDirectory+"\\GilbargoLog.txt",String.Format("SaleCompleted: TotalVolume: {0}, nz.UnitPrice {1}, DecimalPlaces {2}, nz.TotalPrice {3}",totalVolume,nz.UnitPrice,nz.ParentFuelPoint.DecimalPlaces,nz.TotalPrice));
                //nz.CurrentSale.TotalPrice = Math.Round(totalVolume * nz.UnitPrice, nz.ParentFuelPoint.DecimalPlaces);
