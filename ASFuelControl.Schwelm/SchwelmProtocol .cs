...
{
    public class SchwelmProtocol : Common.IFuelProtocol
    {
                ...
                try
                {
                    foreach (Common.FuelPoint fp in this.fuelPoints)
                    {
                        try
                        {
                            ...
                            else if (fp.Status == Common.Enumerators.FuelPointStatusEnum.TransactionCompleted)
                            {
                                this.CloseTransaction(fp);
                            }
        ...
        private bool Halt(FuelPoint fp)
        {
            ...
            if (this.serialPort.BytesToRead > 0)
            {
                ...
                {
                    if (System.IO.File.Exists("Schwelm.log"))
                        System.IO.File.AppendAllText("Schwelm.log", "HALTED : TRUE\r\n");
                    return true;
                }
            }
            if (System.IO.File.Exists("Schwelm.log"))
                System.IO.File.AppendAllText("Schwelm.log", "HALTED : FALSE\r\n");
            return false;
        }
        ...
        public Common.FuelPoint GetStatus(Common.FuelPoint fp)
        {
            ...
            this.serialPort.Write(cmd, 0, cmd.Length);
            if (System.IO.File.Exists("Schwelm.log"))
                System.IO.File.AppendAllText("Schwelm.log", "WRITE GetStatus : \t" + BitConverter.ToString(cmd) + "\r\n");
            int waiting = 0;
            while (this.serialPort.BytesToRead < 3 && waiting < 200)
            {
                waiting += 25;
            ...
            if (System.IO.File.Exists("Schwelm.log"))
                System.IO.File.AppendAllText("Schwelm.log", "READ GetStatus : \t" + BitConverter.ToString(response) + "\r\n");
        ...
        private void JustStatus(Common.FuelPoint fp)
        {
            ...
            if (System.IO.File.Exists("Schwelm.log"))
                System.IO.File.AppendAllText("Schwelm.log", "WRITE JustStatus : \t" + BitConverter.ToString(buffer) + "\r\n");

            this.serialPort.Write(buffer, 0, buffer.Length);
            while (this.serialPort.BytesToRead < 3 && waiting < 300)
            {
                waiting += 25;
                System.Threading.Thread.Sleep(10);
            }
            ...
            if (System.IO.File.Exists("Schwelm.log"))
                System.IO.File.AppendAllText("Schwelm.log", "READ JustStatus : \t" + BitConverter.ToString(trash) + "\r\n");
            Console.WriteLine("\n\n\n\n\n");
        ...
        private bool SetFPtoListen(Common.Nozzle nozzle)
        {
            ...
            if (System.IO.File.Exists("Schwelm.log"))
                System.IO.File.AppendAllText("Schwelm.log", "WRITE SetFPtoListen : \t" + BitConverter.ToString(cmd) + "\r\n");
            ...
            this.serialPort.Read(response, 0, response.Length);
            if (System.IO.File.Exists("Schwelm.log"))
                System.IO.File.AppendAllText("Schwelm.log", "READ SetFPtoListen : \t" + BitConverter.ToString(response) + "\r\n");

            if (response[1] >= 0xD0 && response[1] <= 0xDF)
                return true;
            else
                return false;
        }
        ...
        public bool SetPrice(Common.Nozzle nozzle)
        {
            try
            {
                ...
                if (System.IO.File.Exists("Schwelm.log"))
                    System.IO.File.AppendAllText("Schwelm.log", "WRITE SetPrice : \t" + BitConverter.ToString(cmd) + "\r\n");
                ...
                if (System.IO.File.Exists("Schwelm.log"))
                    System.IO.File.AppendAllText("Schwelm.log", "READ SetPrice : \t" + BitConverter.ToString(response) + "\r\n");
        ...
        private void EvalGetDisplay(Common.Nozzle nozzle, byte[] buffer)
        {
            ...
            try
            {
                ...
                if (System.IO.File.Exists("Schwelm.log"))
                {
                    System.IO.File.AppendAllText("Schwelm.log", "EvalGetDisplay Vol: \t" + nozzle.ParentFuelPoint.DispensedVolume.ToString("N2") + "\r\n");
                    System.IO.File.AppendAllText("Schwelm.log", "EvalGetDisplay Price: \t" + nozzle.ParentFuelPoint.DispensedAmount.ToString("N2") + "\r\n");
                }
        ...
        public void GetDisplayWhileFuelling(Common.Nozzle nozzle)
        {
            ...
            this.serialPort.Write(cmd, 0, cmd.Length);
            if (System.IO.File.Exists("Schwelm.log"))
                System.IO.File.AppendAllText("Schwelm.log", "WRITE GetDisplayWhileFuelling : \t" + BitConverter.ToString(cmd) + "\r\n");
            ...
            this.serialPort.Read(response, 0, response.Length);
            if (System.IO.File.Exists("Schwelm.log"))
                System.IO.File.AppendAllText("Schwelm.log", "READ GetDisplayWhileFuelling : \t" + BitConverter.ToString(response) + "\r\n");
        ...
        public void GetDisplay(Common.Nozzle nozzle)
        {
            ...
            this.serialPort.Write(cmd, 0, cmd.Length);
            if (System.IO.File.Exists("Schwelm.log"))
                System.IO.File.AppendAllText("Schwelm.log", "WRITE GetDisplay : \t" + BitConverter.ToString(cmd) + "\r\n");
            ...
            this.serialPort.Read(response, 0, response.Length);
            if (System.IO.File.Exists("Schwelm.log"))
                System.IO.File.AppendAllText("Schwelm.log", "READ GetDisplay : \t" + BitConverter.ToString(response) + "\r\n");
            EvalGetDisplay(nozzle, response);
        }
        public void CloseTransaction(Common.FuelPoint fp)
        {
            byte startByte = BitConverter.GetBytes(64 + fp.Address)[0];
            byte[] cmd = new byte[] { startByte };
            this.serialPort.Write(cmd, 0, cmd.Length);
            int waiting = 0;
            while (this.serialPort.BytesToRead < 35 && waiting < 300)
            {
                waiting += 25;
                System.Threading.Thread.Sleep(20);
            }
            ...
            if (response.Length < 34)
            {
                ...
                Byte[] response2 = new byte[this.serialPort.ReadBufferSize];
                this.serialPort.Read(response2, 0, response2.Length);
            }
        }
        ...
        public bool AuthorizeFuelPoint(Common.FuelPoint fp)
        {
            ...
            byte[] getSt = CMDGetStatus(fp);
            if (System.IO.File.Exists("Schwelm.log"))
                System.IO.File.AppendAllText("Schwelm.log", "WRITE AuthorizeFuelPoint : \t" + BitConverter.ToString(cmd) + "\r\n");
            ...
            this.serialPort.Read(response, 0, response.Length);
            if (System.IO.File.Exists("Schwelm.log"))
                System.IO.File.AppendAllText("Schwelm.log", "READ AuthorizeFuelPoint : \t" + BitConverter.ToString(response) + "\r\n");
            ...
            this.serialPort.Read(response2, 0, response2.Length);
            if (System.IO.File.Exists("Schwelm.log"))
                System.IO.File.AppendAllText("Schwelm.log", "READ AuthorizeFuelPoint : \t" + BitConverter.ToString(response2) + "\r\n");

            /////////////////////////////////////////////////////////


            this.serialPort.Write(getSt, 0, getSt.Length);
            if (System.IO.File.Exists("Schwelm.log"))
                System.IO.File.AppendAllText("Schwelm.log", "WRITE AuthorizeFuelPoint : \t" + BitConverter.ToString(getSt) + "\r\n");
            System.Threading.Thread.Sleep(25);
            Byte[] response3 = new byte[this.serialPort.ReadBufferSize];
            this.serialPort.Read(response3, 0, response3.Length);
            if (System.IO.File.Exists("Schwelm.log"))
                System.IO.File.AppendAllText("Schwelm.log", "READ AuthorizeFuelPoint : \t" + BitConverter.ToString(response3) + "\r\n");
            ...
            if (response3.Length < 2)
            {
                ...
                if (System.IO.File.Exists("Schwelm.log"))
                    System.IO.File.AppendAllText("Schwelm.log", "write AuthorizeFuelPoint : \t" + BitConverter.ToString(getSt) + "\r\n");
                System.Threading.Thread.Sleep(25);
                Byte[] response4 = new byte[this.serialPort.ReadBufferSize];
                this.serialPort.Read(response4, 0, response4.Length);
                if (System.IO.File.Exists("Schwelm.log"))
                    System.IO.File.AppendAllText("Schwelm.log", "READ AuthorizeFuelPoint : \t" + BitConverter.ToString(response4) + "\r\n");
            }
        ...
        private bool EvalGetTotals(Common.Nozzle nozzle, byte[] response)
        {
            try
            {
                ...
                if (id == nozzle.ParentFuelPoint.Address)
                {
                    ...
                    try
                    {
                        ...
                        if (Math.Abs(totalVolume - nozzle.TotalVolume) > 0 && Math.Abs(diff) > 100 && nozzle.TotalVolume >= 0)
                        {
                            ...
                            if (totalMishMatchIndex < 5)
                            {
                                ...
                                if (System.IO.File.Exists("Schwelm.log"))
                                    System.IO.File.AppendAllText("Schwelm.log", "Total Error Suspected : \t" + diff.ToString("N2") + "\r\n");
                                return false;
                            }
                        }
                    }
                    ...
                    {
                        if (System.IO.File.Exists("Schwelm.log"))
                            System.IO.File.AppendAllText("Schwelm.log", "-----------------------------Internal Exceptiopn Error " + ex.Message + "\r\n");
                        return false;
                    }
                    ...
                    if (System.IO.File.Exists("Schwelm.log"))
                        System.IO.File.AppendAllText("Schwelm.log", "EvalGetTotals : \t" + nozzle.TotalVolume.ToString("N2") + "\r\n");
            ...
            {
                if (System.IO.File.Exists("Schwelm.log"))
                    System.IO.File.AppendAllText("Schwelm.log", e.Message + "\r\n");
                return false;
            }
        ...
        public bool GetTotals(Common.Nozzle nozzle)
        {
            ...
            if (System.IO.File.Exists("Schwelm.log"))
                System.IO.File.AppendAllText("Schwelm.log", "WRITE GetTotals : \t" + BitConverter.ToString(cmd) + "\r\n");
            ...
            this.serialPort.Read(response, 0, response.Length);
            if (System.IO.File.Exists("Schwelm.log"))
                System.IO.File.AppendAllText("Schwelm.log", "READ GetTotals : \t" + BitConverter.ToString(response) + "\r\n");
