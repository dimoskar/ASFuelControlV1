...
namespace ASFuelControl.Data
{
    ...
    partial class Balance
    {
        ...
        //        decimal fillingsRestVol = database.TankFillingInvoiceViews.Where(tfi => 
        //            tfi.TankId == tank.TankId && 
        //            tfi.TransactionTimeEnd <= balance.TimeEnd && 
        //            tfi.TransactionTimeEnd >= balance.TimeStart && 
        //            tfi.InvoiceTypeId != deliveryType && 
        //            (   
        //                tfi.InvoiceTypeId != literCheckType || 
        //                tfi.TransactionType == 1
        //             )).Sum(tfi => tfi.VolumeReal);
        ...
        //        decimal fillingsRestOutVol = database.TankFillingInvoiceViews.Where(tfi =>
        //            tfi.TankId == tank.TankId &&
        //            tfi.TransactionTimeEnd <= balance.TimeEnd &&
        //            tfi.TransactionTimeEnd >= balance.TimeStart &&
        //            tfi.InvoiceTypeId != deliveryType &&
        //            (
        //                tfi.InvoiceTypeId != literCheckType ||
        //                tfi.TransactionType == 0
        //             )).Sum(tfi => tfi.VolumeReal);

        //        decimal fillingsRestOutVol15 = database.TankFillingInvoiceViews.Where(tfi =>
        //            tfi.TankId == tank.TankId &&
        //            tfi.TransactionTimeEnd <= balance.TimeEnd &&
        //            tfi.TransactionTimeEnd >= balance.TimeStart &&
        //            tfi.InvoiceTypeId != deliveryType &&
        ...
        //                tfi.InvoiceTypeId != literCheckType ||
        //                tfi.TransactionType == 0
        //             )).Sum(tfi => tfi.VolumeRealNormalized);
        ...
        //        fillingRest15.Add(tank, fillingsRestOutVol);
        //        fillingRestOut.Add(tank, fillingsRestOutVol15);
        ...
        //            ftpc.TotalSalesNormalized = qs.ToList().Sum(s => s.VolumeNormalizedCalc) - ftpc.TotalLiterCheckNormalized;
        ...
        //        mov.SumAdditionalIn = fillingRest.Where(t => t.Key.FuelTypeId == ft1.FuelTypeId).Sum(t => t.Value) - fillingRestOut.Where(t => t.Key.FuelTypeId == ft1.FuelTypeId).Sum(t => t.Value);                      //F_2236A2
        //        mov.SumInNormalized = fillingsDelivery15.Where(t => t.Key.FuelTypeId == ft1.FuelTypeId).Sum(t => t.Value);              //F_2236B1
        //        mov.SumAdditionalInNormalized = fillingRest15.Where(t => t.Key.FuelTypeId == ft1.FuelTypeId).Sum(t => t.Value) - fillingRestOut.Where(t => t.Key.FuelTypeId == ft1.FuelTypeId).Sum(t => t.Value);           //F_2236B2
        ...
        //        var q = database.SystemEvents.Where(s => s.AlarmType == (int)Common.Enumerators.AlarmEnum.BalanceDifference && !s.ResolvedDate.HasValue).ToList();
        //        foreach (SystemEvent se in q)
        //        {
        //            if(database.AlertChecker != null)
        //                database.AlertChecker.ResolveAlert(se.EventId, "[Date Resolved]");
        //        }
        //        if (database.AlertChecker != null)
        //        {
        ...
        //            if (database.AlertChecker != null)
        //            {
        //                string message = string.Format("ΗΜΕΡΗΣΙΟ ΙΣΟΖΥΓΙΟ ΕΚΤΟΣ ΟΡΙΩΝ {0:N2}% ΣΤΟ ΚΑΥΣΙΜΟ : {1}({2})", div.PercentageNormalized, ft1.Name, ft1.Code);
        //                database.AlertChecker.CreateBalanceAlert(database, string.Format("{0} - {1}", ft1.Name, ft1.Code), div.PercentageNormalized, message, Common.Enumerators.AlarmEnum.BalanceDifference, balance.Date, sales);
        //            }
    ...
    public partial class Invoice
    {
        ...
        public decimal DiscountNettoAmount
        {
            get
            {
                ...
                decimal disc = Math.Round((this.DiscountAmount.HasValue ? this.DiscountAmount.Value : 0) / vatRatio, 2);
                return disc;
            }
        }
        ...
        public decimal DiscountPercentage
        {
            get
            {
                if (this.SalesTransaction == null)
                    return 0;
                return this.SalesTransaction.DiscountPercentage.HasValue ? this.SalesTransaction.DiscountPercentage.Value : 0;
            }
        }
    ...
    public partial class SystemEvent
    {
        ...
        //            return "ΗΜΕΡΗΣΙΟ ΙΣΟΖΥΓΙΟ ΕΚΤΟΣ ΟΡΙΩΝ {0:N2}% ΣΤΟ ΚΑΥΣΙΜΟ: {1}. Σύνολο Πωλήσεων: {2:N2}Lt";
