...
namespace nuovoPignone
{
    public class FuelPoint
    {
            ...
            try
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (this.Status == Common.Enumerators.FuelPointStatusEnum.Idle)
                        {
                            if (this.ActiveNozzle != null)
                            {
                        ...
                        else if (this.status == Common.Enumerators.FuelPointStatusEnum.Nozzle)
                        {
                            ...
                            if (this.QueryAuthorize)
                                nextCommand = this.AuthoriseFuelPoint(this.ActiveNozzle.UnitPrice, (decimal)9999.99, 999999);
                            else if (this.QueryTotals)
                                nextCommand = this.GetNozzleTotals(this.ActiveNozzle);
                            else
                                nextCommand = this.GetFuelPointStatus();
        ...
        private OpenCommand GetFuelPointStatus()
        {
        ...
        private OpenCommand InitializeFuelPoint()
        {
            byte startByte = BitConverter.GetBytes(240 + this.AddressId - 1)[0];
        ...
        private OpenCommand AuthoriseFuelPoint(decimal price, decimal presetMoney, decimal presetVolume)
        {
            byte startByte = BitConverter.GetBytes(240 + this.AddressId - 1)[0];
            List<Byte> commandBuffer = new List<byte>();
            ...
            byte[] prBuffer = this.GetDecimalBytes((decimal)9999.99, this.VolumeDecimalPlaces, 3);
            byte[] volBuffer = this.GetDecimalBytes((decimal)9999.99, this.VolumeDecimalPlaces, 3);
        ...
        private OpenCommand GetActiveNozzle()
        {
            byte startByte = BitConverter.GetBytes(192 + this.AddressId - 1)[0];
            List<Byte> commandBuffer = new List<byte>();
        ...
        private OpenCommand StopFuelPoint()
        {
            byte startByte = BitConverter.GetBytes(240 + this.AddressId - 1)[0];
            byte startByteNeg = (byte)(255 - startByte);
