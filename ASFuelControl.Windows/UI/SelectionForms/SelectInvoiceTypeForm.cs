...
namespace ASFuelControl.Windows.UI.SelectionForms
{
    public partial class SelectInvoiceTypeForm : RadForm
    {
        ...
        public SelectInvoiceTypeForm()
        {
            ...
            this.Disposed += SelectInvoiceTypeForm_Disposed;
            
        }

        private void SelectInvoiceTypeForm_Disposed(object sender, EventArgs e)
        {
            this.db.Dispose();
        }
        ...
        {
            var q = this.db.InvoiceTypes.Where(i => !i.Invalidated.HasValue || !i.Invalidated.Value);
            if (this.HasFinancialTransactions)
            {
                this.invoiceTypeBindingSource.DataSource = q.Where(it => it.TransactionType == 0 && it.InvoiceTypeId != literCheck &&
                        (
                ...
                if (trader != null)
                {
                    this.invoiceTypeBindingSource.Position = q.Where(it => it.TransactionType == this.TransactionType &&
            ...
            else
            {
                ...
                if (trader != null)
                {
                    this.invoiceTypeBindingSource.DataSource = q.Where(it => it.TransactionType == 0 && it.InvoiceTypeId != literCheck && it.TransactionSign >= 0 &&
                            (
                                (it.IsInternal.HasValue && it.IsInternal.Value && this.InternalType == 1) ||
                                ((!it.IsInternal.HasValue || !it.IsInternal.Value) && this.InternalType == 0) ||
                                (this.InternalType == -1)
                    ...
                    this.invoiceTypeBindingSource.Position = q.Where(it => it.TransactionType == this.TransactionType &&
                        it.InvoiceTypeId != literCheck && it.TransactionSign >= 0 &&
                        (
                            (it.IsInternal.HasValue && it.IsInternal.Value && this.InternalType == 1) ||
                            ((!it.IsInternal.HasValue || !it.IsInternal.Value) && this.InternalType == 0) ||
                            (this.InternalType == -1)
