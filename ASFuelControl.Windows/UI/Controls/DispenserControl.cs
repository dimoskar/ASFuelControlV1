    ...
    public partial class DispenserControl : UserControl
    {
        ...
        public VirtualDevices.VirtualDispenser Dispenser
        {
            set
            {
                ...
                this.labelSerial.Text = dispenser.SerialNumber;
        ...
        public void SetFleetSalesImage()
        {
            if (this.InvokeRequired)
            {
                this.Invoke(new Action(() => this.SetFleetSalesImage()));
                return;
            }
            if (this.dispenser.FleetSales)
                this.panelFleetSales.BackgroundImage = Properties.Resources.CardEnabled;
            else
                this.panelFleetSales.BackgroundImage = Properties.Resources.CardDisabled;
        }

        public void SetTraderButtonColor(Color color)
        {
            if (this.InvokeRequired)
            {
                this.Invoke(new Action(() => this.SetTraderButtonColor(color)));
                return;
            }
            this.btnCustomer.BackColor = color;
        }
        ...
        public void RefreshValues()
        {
            if (this.InvokeRequired)
            {
                this.Invoke(new Action(() => this.RefreshValues()));
                return;
            }

            if (this.Dispenser.FleetDispensers.Length == 0)
            {
                this.panelFleetSales.Hide();
            }

            foreach(VirtualDevices.VirtualFleetDispenser vfd in this.Dispenser.FleetDispensers)
            {
                if (vfd.ShouldSetStateOff)
                    this.Dispenser.FleetSales = false;
                else if(vfd.ShouldSetStateOn)
                    this.Dispenser.FleetSales = true;
            }
            ...
            if (this.dispenser.ActiveNozzle != null)
            {
                ...
                this.labelNozzleNumber.Text = this.dispenser.ActiveNozzle.NozzleOfficialNumber.ToString();
        ...
        void dispenser_PropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            try
            {
                ...
                if (e.PropertyName == "UpdateUI" && this.dispenser.UpdateUI)
                    this.RefreshValues();
                if (e.PropertyName == "NextSaleTrader")
                {
                    if (this.dispenser.NextSaleTrader == null)
                        this.SetTraderButtonColor(Color.Transparent);
                    else
                        this.SetTraderButtonColor(Color.Orange);
                }
                else if (e.PropertyName == "FleetSales")
                {
                    this.SetFleetSalesImage();
                }
            }
        ...
        {
            using (UI.SelectionForms.SelectNozzleForm snf = new SelectionForms.SelectNozzleForm())
            {
        ...
        private void btnCustomer_Click(object sender, EventArgs e)
        {
            if (this.Dispenser.NextSaleVehicle != Guid.Empty)
            {
                ...
                this.Dispenser.Odometer = 0;
                //this.btnCustomer.BackColor = Color.Transparent;
            }
            else
            {
                using (SelectionForms.VehicleSelectForm stf = new SelectionForms.VehicleSelectForm())
                {
                    
                    stf.ShowDialog(this);
                    if (stf.SelectedEntity == null)
                        return;
                    var vehicle = stf.SelectedEntity as ViewModels.VehicleViewModel;
                    if (vehicle == null)
                        return;
                    using (SelectionForms.InvoiceTypeSelection sif = new SelectionForms.InvoiceTypeSelection())
                    {
                        sif.SelectedInvoiceTypeId = vehicle.DefaultInvoiceTypeId;
                        sif.DispenserTypes = true;
                        var res = sif.ShowDialog(this);
                        if (res == DialogResult.Cancel)
                            return;

                        ViewModels.InvoiceTypeViewModel ivm = sif.SelectedEntity as ViewModels.InvoiceTypeViewModel;
                        if (ivm == null)
                            return;
                        this.Dispenser.InvoiceTypeId = ivm.InvoiceTypeId;
                        this.Dispenser.NextSaleVehicle = vehicle.VehicleId;
                        this.Dispenser.NextSaleTrader = vehicle.TraderId;
                        this.Dispenser.Odometer = vehicle.Odometer;
                        //this.btnCustomer.BackColor = Color.Orange;
                    }
                }
            }
        ...
        {
            UI.Forms.DispenserDiscountForm ddf = new Forms.DispenserDiscountForm();
            ddf.CurrentVirtualDispenser = this.dispenser;
            ddf.CurrentDispenser = Program.ApplicationMainForm.ThreadControllerInstance.GetDispenser(this.dispenser);
            ddf.Show();
        }

        private void panelFleetSales_Click(object sender, EventArgs e)
        {
            this.dispenser.FleetSales = !this.dispenser.FleetSales;
        }
    }
}
