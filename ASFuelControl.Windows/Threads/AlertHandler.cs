    ...
    public class AlertHandler
    {
        ...
        public void CheckForAlerts()
        {
            ...
            if (gcIndex >= 100)
            {
                ...
                {
                    database.Dispose();
                    database = new Data.DatabaseModel(Properties.Settings.Default.DBConnection);
        ...
        public void CheckForTankAlerts(VirtualDevices.VirtualTank tank)
        {
            ...
            try
            {
                ...
                {
                    //if (currentStatus == Common.Enumerators.TankStatusEnum.Selling)
                    //    return;
                    ...
                    foreach (Common.Enumerators.TankStatusEnum alertStatus in aStatuses)
                    {
                        ...
                        if (alertStatus == Common.Enumerators.TankStatusEnum.LevelDecrease && DateTime.Now.Subtract(tank.LastSaleEnd).TotalSeconds < 10)
                            ...
                            tank.AlertStatuses.Remove(alertStatus);
                        else if (alertStatus == Common.Enumerators.TankStatusEnum.LevelDecrease && 
                                (tank.TankStatus == Common.Enumerators.TankStatusEnum.FuelExtractionInit || 
                                tank.TankStatus == Common.Enumerators.TankStatusEnum.FuelExtraction || 
                                tank.TankStatus == Common.Enumerators.TankStatusEnum.FillingInit || 
                                tank.TankStatus == Common.Enumerators.TankStatusEnum.Filling)
                            )
                            tank.AlertStatuses.Remove(alertStatus);
            ...
            finally
            {
                foreach (Common.Enumerators.TankStatusEnum alertStatus in tank.AlertStatuses)
                {
                    ...
                    if (alarm == null)
                    {
                        this.ResetPeriodParameters(tank);
                        continue;
                    }
                    alerts.Add(alarm);
                }
                ...
                foreach (VirtualDevices.VirtualTankAlarm oldAlarm in tank.Alerts)
                {
                    ...
                    if (newAlarm != null)
                    {
                        if (newAlarm.AlertType == Common.Enumerators.AlarmEnum.FuelIncrease || newAlarm.AlertType == Common.Enumerators.AlarmEnum.FuelDecrease)
                        {
                            Data.SystemEvent se = this.database.SystemEvents.Where(s => s.EventId == oldAlarm.DatabaseEntityId).FirstOrDefault();
                            if (se != null)
                            {
                                se.UpdateData("CurrentFuelLevel", tank.CurrentFuelLevel.ToString("N2"));
                                se.UpdateData("LastFuelLevel", tank.LastFuelHeight.ToString("N2"));
                                se.UpdateData("CurrentTemperature", tank.CurrentTemperature.ToString("N1"));
                                se.UpdateData("LastTemperature", tank.LastTemperature.ToString("N1"));
                                se.UpdateData("CurrentVolume", tank.GetTankVolume(tank.CurrentFuelLevel).ToString("N2"));
                                se.UpdateData("LastVolume", tank.GetTankVolume(tank.LastFuelHeight).ToString("N2"));
                                oldAlarm.UpdateData("CurrentFuelLevel", tank.CurrentFuelLevel.ToString("N2"));
                                oldAlarm.UpdateData("LastFuelLevel", tank.LastFuelHeight.ToString("N2"));
                                oldAlarm.UpdateData("CurrentTemperature", tank.CurrentTemperature.ToString("N1"));
                                oldAlarm.UpdateData("LastTemperature", tank.LastTemperature.ToString("N1"));
                                oldAlarm.UpdateData("CurrentVolume", tank.GetTankVolume(tank.CurrentFuelLevel).ToString("N2"));
                                oldAlarm.UpdateData("LastVolume", tank.GetTankVolume(tank.LastFuelHeight).ToString("N2"));
                                if(this.AlarmUpdated != null)
                                    this.AlarmUpdated(oldAlarm, new EventArgs());
                            }
                        }
                        continue;
                    }
                ...
                foreach (VirtualDevices.VirtualTankAlarm tAlarm in alerts)
                {
                    ...
                    {
                        tank.ErrorStatus = Common.Enumerators.TankStatusEnum.LevelIncrease;
                    }
                    ...
                    {
                        tank.ErrorStatus = Common.Enumerators.TankStatusEnum.HighWaterLevel;
                    }
                }
                //if(alerts.Count == 0)
        ...
        private VirtualDevices.VirtualTankAlarm CreateTankAlert(VirtualDevices.VirtualTank tank, Common.Enumerators.TankStatusEnum status)
        {
            ...
            {
                if (Math.Abs(tank.LastFuelHeight - tank.CurrentFuelLevel) < 1)
                    return null;
        ...
        {
            Data.Tank tdb = this.database.Tanks.Where(t => t.TankId == tank.TankId).FirstOrDefault();
            if (DateTime.Now.Subtract(tdb.MinimumSetTime).TotalSeconds < 60)
                return false;

            Data.TankFilling tf = this.database.TankFillings.Where(t=>t.TankId == tdb.TankId).OrderBy(t => t.TransactionTimeEnd).Last();
            DateTime dtFrom = new DateTime(2000,1,1);
            if (tf == null)
                return false;
            dtFrom = tf.TransactionTimeEnd;
            decimal tankVolume = tdb.GetTankVolume(tf.LevelEnd);
            decimal tankVolumeNormalized = tank.NormalizeVolume(tankVolume, tank.CurrentTemperature, tank.CurrentDensity);
            decimal totalSalesNormalized = this.database.SalesTransactions.Where(s =>
                s.TankSales.Where(t => t.TankId == tdb.TankId).Count() > 0 &&
                s.TransactionTimeStamp > dtFrom).Sum(s => s.VolumeNormalized);
            ...
            decimal diff = Math.Abs((tankVolumeNormalized - totalSalesNormalized) - currentNormalizedVol);
            if (diff > ((decimal)1.5 * totalSalesNormalized) / 100)
                return true;
            return false;

            //decimal currentVolume = tank.GetTankVolume(tank.CurrentFuelLevel);
            ////decimal lastVolume = tank.GetTankVolume(tank.LastFuelHeight);
            //decimal lastVolume = tank.GetTankVolume(tdb.PeriodMaximum);//tank.GetTankVolume(tdb.GetLastValidLevel());// tank.GetTankVolume(tank.LastFuelHeight);
            //decimal currentNormalizedVol = tank.NormalizeVolume(currentVolume, tank.CurrentTemperature, tank.CurrentDensity);
        ...
        private bool IsIncreasing(VirtualDevices.VirtualTank tank, Common.Enumerators.TankStatusEnum currentStatus)
        {
            ...
            Data.Tank tdb = this.database.Tanks.Where(t => t.TankId == tank.TankId).FirstOrDefault();
            decimal currentVolume = tank.GetTankVolume(tank.CurrentFuelLevel);
            decimal lastVolume = tank.GetTankVolume(tdb.PeriodMinimum);//tdb.GetLastValidLevel());// tank.GetTankVolume(tank.LastFuelHeight);
            decimal currentNormalizedVol = tank.NormalizeVolume(currentVolume, tank.CurrentTemperature, tank.CurrentDensity);
            decimal lastNormalizedVol = tank.NormalizeVolume(lastVolume, tank.LastTemperature, tank.CurrentDensity);
        ...
        private void ResetPeriodParameters(VirtualDevices.VirtualTank tank)
        {
            Data.Tank dbTank =  this.database.Tanks.Where(t => t.TankId == tank.TankId).FirstOrDefault();
            if (dbTank == null)
                return;
            dbTank.ResetPeriodParameters();
        }
