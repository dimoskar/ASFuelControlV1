...
namespace ASFuelControl.Windows.Threads
{
    ...
    /// <summary>
    /// Class providing functionality for diret printing on thermal printers.
    /// </summary>
    public class DirectPrinter
    {
        ...
        public static void SendToPrinter(string jobName, string PCL5Commands, string printerName)
        {
            ...
            //byte[] uni = Encoding.Default.GetBytes(PCL5Commands);

            //// Convert to ASCII
            //byte[] ascBuf = Encoding.Convert(Encoding.Default, Encoding.GetEncoding(437), uni);
            //string Ascii = Encoding.GetEncoding(437).GetString(ascBuf);

            bool microFont = Data.Implementation.OptionHandler.Instance.GetBoolOption("PrinterMicroFont", false);
            string command = PCL5Commands;
            if(microFont)
            {
                string ESC = Convert.ToString((char)27);
                command = ESC + "M1" + command;
            ...
            WritePrinter(lhPrinter, command, command.Length, ref pcWritten);
