...
namespace ASFuelControl.PumalanRelay
{
    public class PumalanProtocol : Common.IFuelProtocol
    {
        ...
        private System.IO.Ports.SerialPort serialRelay = new System.IO.Ports.SerialPort();
        ...
        public bool IsRelayConnected
        {
            get
            {
                return this.serialRelay.IsOpen;
            }
        }
        ...
        private Dictionary<string, string> RelaySettings = new Dictionary<string, string>();
        public void GetSettings(string path)
        {
            if (RelaySettings.Count > 0)
                RelaySettings.Clear();
            XmlDocument doc = new XmlDocument();
            doc.Load(path);
            XmlNodeList XmlSection = doc.GetElementsByTagName("setting");

            for (int i = 0; i < XmlSection.Count; i++)
            {
                string ParameterName = XmlSection[i].Attributes["Name"].Value;
                string ParameterValue = XmlSection[i].Attributes["Value"].Value;
                RelaySettings.Add(ParameterName, ParameterValue);
            }

        }
        public void createXML(string SavePath)
        {
            string XML = "<ParameterDBConfig>"
                + "<setting Name =\"RelayPort\" Value=\"COM4\" />"
                + "<setting Name =\"RelayBaudRate\" Value=\"9600\" />"
                + "<setting Name =\"RelayController\" Value=\"Enable\" />"
                + "</ParameterDBConfig>";
            if (!System.IO.File.Exists(SavePath))
            {
                System.IO.File.Create(SavePath);
                System.IO.TextWriter FileCreator = new System.IO.StreamWriter(SavePath);
                FileCreator.WriteLine(XML);
                FileCreator.Close();
            }
        }
        public void Connect()
        {
            try
            {
                if (!System.IO.File.Exists(@"C:\ASFuelControl\Pumalan.xml"))
                {
                    createXML(@"C:\ASFuelControl\Pumalan.xml");
                }
                else
                { 
                    GetSettings(@"C:\ASFuelControl\Pumalan.xml");
                }
                string RelayCOMPort, RelayBaudRate;
                
                RelaySettings.TryGetValue("RelayPort", out RelayCOMPort);
                RelaySettings.TryGetValue("RelayBaudRate", out RelayBaudRate);

                this.serialRelay.PortName = RelayCOMPort;
                this.serialRelay.BaudRate = int.Parse(RelayBaudRate);
                this.serialRelay.Parity = Parity.None;
        ...
        public void Disconnect()
        {
            ...
            {
                if (this.serialRelay.IsOpen)
                    this.serialRelay.Close();
        ...
        private bool MotorAuthorised = false, MotorWorked = false;
        private void ThreadRun()
        {
            ...
            try
            {
                ...
                #region Loopa Kleisimatos Seriakwn
                if (!this.serialRelay.IsOpen)
                {
                    this.serialRelay.Open();
                }
                this.serialRelay.Write(CreateCommandRelay(RelayAction.Close, RelayMotor.Relay1), 0, 3);
                    ...
                    try
                    {
                        ...
                        foreach (Common.FuelPoint fp in this.fuelPoints)
                        {
                            ...
                            try
                            {
                                ...
                                if (MotorAuthorised && !MotorWorked)
                                {
                                    if (!this.serialRelay.IsOpen)
                                    {
                                        this.serialRelay.Open();
                                    }
                                    this.serialRelay.Write(CreateCommandRelay(RelayAction.Open, RelayMotor.Relay1), 0, 3);
                                    System.Threading.Thread.Sleep(10);
                                    this.serialRelay.Write(CreateCommandRelay(RelayAction.Open, RelayMotor.Relay1), 0, 3);
        ...
        private byte[] CreateCommandRelay(RelayAction SelectCommand, RelayMotor SelectMotor)
        {
            int SMotor = (int)SelectMotor;
            int Action = (int)SelectCommand;
            byte[] Buf;
            if (Action < 2)
                Buf = new byte[] { 0xFF, (byte)SMotor, (byte)Action };
            else
                Buf = new byte[] { 0xFF, (byte)Action, 00 };
        ...
        private Common.Nozzle evalDisplay(Common.Nozzle nozzle, byte[] response)
        {
            ...
            if (response.Length > 15)
            {
                ...
                nozzle.ParentFuelPoint.DispensedAmount = decimal.Parse(timi) / (decimal)Math.Pow(10, nozzle.ParentFuelPoint.AmountDecimalPlaces);
