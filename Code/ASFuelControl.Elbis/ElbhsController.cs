            ...
            foreach (FuelPoint fp in this.fuelPoints)
            {
                ...
                switch (newStatus)
                {
                    ...
                    case Common.Enumerators.FuelPointStatusEnum.Work:
                        fp.ActiveNozzle = fp.Nozzles[values.ActiveNozzle];
                        values.CurrentPriceTotal = (decimal)MyCom.Channel[fp.Channel + nozzle - 1].Delivery.CurrentAmount / (decimal)Math.Pow(10, fp.CommonFP.DecimalPlaces);
                        ...
                        values.CurrentVolume = (decimal)MyCom.Channel[fp.Channel + nozzle - 1].Delivery.CurrentVolume / (decimal)Math.Pow(10, fp.CommonFP.DecimalPlaces);
                        fp.DispenserStatus = newStatus;
                        this.EnqueValues(fp.CommonFP, values);
                        break;
