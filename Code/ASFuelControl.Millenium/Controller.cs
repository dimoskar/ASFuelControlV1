...
namespace ASFuelControl.Millenium
{
    public class Controller : Common.FuelPumpControllerBase
    {
        public Controller()
        {
            this.ControllerType = Common.Enumerators.ControllerTypeEnum.Millenium;
            this.Controller = new MilleniumProtocol();
        }
        protected override void OnTransactionCompleted(Common.TotalsEventArgs e)
        {
            ...
            //
                ...
                decimal fpTotal = (decimal)fp.DispensedVolume / (decimal)Math.Pow(10, fp.DecimalPlaces);
                if(totalVolume != fpTotal)
                    nz.CurrentSale.TotalPrice = Math.Round(totalVolume * nz.UnitPrice, nz.ParentFuelPoint.DecimalPlaces);
                else
                    nz.CurrentSale.TotalPrice = (decimal)fp.DispensedAmount / (decimal)Math.Pow(10, fp.DecimalPlaces);
                nz.CurrentSale.SaleEndTime = DateTime.Now;

                Common.FuelPointValues values = new Common.FuelPointValues();
                values.Status = fp.DispenserStatus;
                values.ActiveNozzle = fp.ActiveNozzleIndex;
                values.CurrentPriceTotal = nz.CurrentSale.TotalPrice;
                values.CurrentSalePrice = nz.CurrentSale.UnitPrice;
                values.CurrentVolume = nz.CurrentSale.TotalVolume;
