...
namespace ASFuelControl.PTS
{
    public class PtsConnector : Common.IController
    {
        ...
        void fp_TotalsUpdated(object sender, PTSLib.PTS.TotalsEventArgs e)
        {
            ...
            {
                if (e.Nozzle.TotalDispensedVolume == 0 && nz.CurrentSale.TotalizerStart > 0)
                {
                    nz.QueryTotals = true;
                    return;
                }
                nz.CurrentSale.TotalizerEnd = e.Nozzle.TotalDispensedVolume;
                if (nz.CurrentSale.TotalizerEnd == nz.CurrentSale.TotalizerStart)
                {
                    if (e.Nozzle.FuelPoint.DispensedVolume > 0)
                    {
                        decimal lastTotal = nz.GetLastTotalizer();
                        if (lastTotal == -1)
                        {
                            nz.CurrentSale.TotalizerStart = nz.CurrentSale.TotalizerStart - (e.Nozzle.FuelPoint.DispensedVolume);
                        }
                        else
                        {
                            if(nz.CurrentSale.TotalizerStart > lastTotal)
                                nz.CurrentSale.TotalizerStart = lastTotal;
                            else
                                nz.CurrentSale.TotalizerStart = nz.CurrentSale.TotalizerStart - (e.Nozzle.FuelPoint.DispensedVolume );
                        }   
                    }
                ...
                //nz.CurrentSale.TotalizerStart = nz.CurrentSale.TotalizerEnd;
                decimal totalVolume = (nz.CurrentSale.TotalizerEnd - nz.CurrentSale.TotalizerStart) / 100;
                if (nz.CurrentSale.TotalizerStart < 0)
                {
                    nz.CurrentSale.TotalizerStart = e.Nozzle.TotalDispensedVolume;
                    nz.QueryTotals = false;
                    return;
                }
                if (totalVolume > 1000 || totalVolume <= 0)
                {
                ...
                //nz.CurrentSale.TotalizerStart = e.Nozzle.LastTotalDispensedVolume;
                
                nz.CurrentSale.TotalVolume = totalVolume;
                nz.CurrentSale.TotalPrice = (decimal)fuelPoint.DispensedAmount / (decimal)Math.Pow(10, fp.DecimalPlaces);
                decimal fpPrice = Math.Round(totalVolume * nz.UnitPrice, nz.ParentFuelPoint.DecimalPlaces);
                if( Math.Abs(fpPrice - nz.CurrentSale.TotalPrice) > (decimal)0.02)
                {
                     nz.CurrentSale.TotalPrice = Math.Round(totalVolume * nz.UnitPrice, nz.ParentFuelPoint.DecimalPlaces);
                }
        ...
        void fp_DispensedValuesChanged(object sender, EventArgs e)
        {
            ...
            values.CurrentPriceTotal = (decimal)fp.DispensedAmount / (decimal)Math.Pow(10, fuelPoint.DecimalPlaces); ;
