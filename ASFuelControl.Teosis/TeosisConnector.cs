...
namespace ASFuelControl.Teosis
{
    public class Controller : Common.FuelPumpControllerBase
    {
        ...
        public class TeosisConnector : IFuelProtocol
        {
            ...
            {
                var cmd = this.CreateCommand(fp, ExecutionCommand.GETPUMPTOTALS);
                if (cmd == null)
                    return false;
                cmd.NozzleIndex = nozzleIdx;
                cmd.CreateCommand(0);
            ...
            //private void CreateSale(Common.FuelPoint fp)
            //{
            //    var nozzle = fp.ActiveNozzle != null ? fp.ActiveNozzle : fp.LastActiveNozzle;
            //    if (nozzle.NozzleIndex > 0)
            //    {
            //        var ifp = this.InternalPumps.First(p => p.Address == fp.Address);
            //        var inz = ifp.Nozzles.First(n => n.NozzleIndex == nozzle.NozzleIndex);
            //        var nz = currentCommand.FuelPump.Nozzles[nozzle.NozzleIndex - 1];
            //        nz.TotalVolume = inz.TotalVolume;
            //        nz.TotalPrice = inz.TotalPrice;

            //        fp.NeedsInvoice = true;
            //        fp.SetExtendedProperty("SaleInitiated", true);
            //        var sale = new SaleEventArgs
            //            (fp,
            //                nozzle.NozzleIndex,
            //                inz.TotalVolume,
            ...
            //                fp.DispensedAmount,
            //                fp.DispensedVolume);
            //        fp.SetExtendedProperty("LastSale", sale);
            //    }
            //}
                    ...
                    try
                    {
                        ...
                        {   
                            ...
                            if (reportStatus == 0)
                            {
                                ...
                                {
                                    int count = (int)fp.GetExtendedProperty("ReportStatusCount", 0);
                                    if (count % 2 == 0 && count > 0)
                                    {
                                        System.Threading.Thread.Sleep(2000);
                                        continue;
                                    }
                                    fp.SetExtendedProperty("ReportStatusCount", count + 1);
                                    continue;
                                }
                                System.Threading.Thread.Sleep(100);
                            }

                            fp.SetExtendedProperty("ReportStatusCount", 0);
    ...
    public class InternalPump
    {
        ...
        public void InitializePump(Common.FuelPoint pump)
        {
            ...
            foreach (var nz in pump.Nozzles)
            {
                ...
                n.VolumeDecimalPlaces = pump.VolumeDecimalPlaces;
                n.TotalDecimalPlaces = pump.TotalDecimalPlaces;
                nzList.Add(n);
            }
    ...
    public class InternalNozzle
    {
        ...
        public decimal TotalVolume
        {
            get
            {
                var initVol = this.InitialVolume / (decimal)Math.Pow(10, TotalDecimalPlaces);
                var sales = this.SalesVolume / (decimal)Math.Pow(10, VolumeDecimalPlaces);
                var volume = (initVol + sales) * (decimal)Math.Pow(10, TotalDecimalPlaces);
                return volume;// decimal.Round(this.SalesVolume / (decimal)Math.Pow(10, VolumeDecimalPlaces), VolumeDecimalPlaces);
            }
        }
